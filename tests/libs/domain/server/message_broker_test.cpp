...
{
protected:
    bc::protocol::MailboxID aliceId;
    bc::protocol::MailboxID bobId;
...
TEST_F(MessageBrokerTest, PushAndPollReconstructsFrameProperly)
{
    ...
    EXPECT_EQ(frameOut.GetPayload(), (bc::protocol::Payload{0x01, 0x02, 0x03}));
...
TEST_F(MessageBrokerTest, MessagesAreProcessedInStrictFIFOOrder)
{
    ...
    EXPECT_EQ(res3.value().GetPayload(), (bc::protocol::Payload{0x33}));
